...
const parse_atom = (pdb_line) => {
	return {
        serial: 
			parseInt(pdb_line.substring(6, 11)),
        name: 
			pdb_line.substring(12, 16).trim(),
        alt_loc: 
			pdb_line.substring(16, 17).trim(),
        res_name: 
			pdb_line.substring(17, 20).trim(),
        chain_id: 
			pdb_line.substring(21, 22).trim(),
        res_seq: 
			parseInt(pdb_line.substring(22, 26)),
        i_code: 
			pdb_line.substring(26, 27).trim(),
        x: 
			parseFloat(pdb_line.substring(30, 38)),
        y: 
			parseFloat(pdb_line.substring(38, 46)),
        z: 
			parseFloat(pdb_line.substring(46, 54)),
        occupancy: 
			parseFloat(pdb_line.substring(54, 60)),
        temp_factor: 
			parseFloat(pdb_line.substring(60, 66)),
        element: 
			pdb_line.substring(76, 78).trim(),
        charge: 
			parse_charge(pdb_line.substring(78, 80).trim()),
      }
}
